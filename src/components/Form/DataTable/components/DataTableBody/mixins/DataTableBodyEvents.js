...
export const DataTableBodyEvents = {
    ...
    isAddingMode() {
      if (this.isAddingMode) {
        ...
        if (this.activeElement) {
          targetInsert = document.querySelector(`.${this.guid} .body [data-rowid="${this.activeElement.id}"]`);
        }
  ...
  methods: {
    ...
    eventColumnDblclick(event, element, fieldOption, fieldValue) {
      if (this.isAddingMode) { event.preventDefault(); return; }
      // console.log(itemRow);
    ...
    mountFormEditField(option) {
      ...
      const subClassVue = Vue.extend(FormEditField);
      ...
      this.formEditField.$on('form:update-field', (options) => {
        this.$emit('form:update-field', options);
      });
    },
    ...
    mountFormEditElement(option) {
      ...
      this.formEditElement.$on('form:mark-mode-adding', () => {
        this.$emit('form:mark-mode-adding');
      });
      this.formEditElement.$on('form:mark-mode-adding-off', () => {
        this.$emit('form:mark-mode-adding-off');
      });
      this.formEditElement.$on('form:adding', (formData) => {
        this.$emit('form:adding', formData);
      });
    },
    ...
    eventMouseOver(event) {
      ...
      if (!this.isColumnFocus && !this.isColumnEditing && !this.isRowFocus && !this.isAddingMode)
