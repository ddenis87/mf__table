export const DataTableEvents = {
  methods: {
    ...
    focusedElement(event, value) {
      ...
      // console.log(option);
      this.eventComponent('element:focused', value)
      // this.$emit('event-row-focused', option);
    },
    selectedElement(event, value) {
      // this.$store.commit('DataTable/SET_ACTIVE_ELEMENT', Object.assign({ value: value }, this.optionGetter));
      // console.log(option);
      this.eventComponent('element:selected', value)
      // this.$emit('event-row-selected', option, this.properties.tableName);
    },
    evtMarkModeAdding() {
      this.eventComponent('form:mark-mode-adding');
    },
    evtMarkModeAddingOff() {
      this.eventComponent('form:mark-mode-adding-off');
    },
    evtFormAdding(formData) {
      this.eventComponent('form:adding', formData);
    },
    evtFormUpdateField(options) {
      this.eventComponent('form:update-field', options);
    },
