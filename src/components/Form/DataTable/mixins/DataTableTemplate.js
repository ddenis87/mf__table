export const DataTableTemplate = {
  computed: {
    headers() {
      return (this.isMultiline) ? this.options.templateMultiline : this.options.templateUno;
    },
    listHeaders() {
      if (!this.headers) return [];
      const listHeaders = [];
      let prepareHeaders = this.headers;
      if (Array.isArray(this.headers[0])) {
        prepareHeaders = [];
        for (let i = 0; i < this.headers.length; i++) { prepareHeaders.push(...this.headers[i]) }
      }
      prepareHeaders.forEach((element) => {
        if (Object.keys(this.listOptions).includes(element.value)) {
          listHeaders.push({
            ...element,
            ...this.listOptions[element.value],
            'position_in_template': {
              'grid-area': element.value,
            },
          });
        }
      });
    ...
    dataTableTemplate() {
      ...
      if (!this.headers) return template;
      ...
      if (!headers[0].length) {
        ...
        template['grid-template-rows'] = `repeat(${headers.length}, ${(this.typeRow == 'fixed') ? '43px' : (this.typeRow == 'dense') ? '22px' : 'auto'})`;
      }
      return template;
    },
