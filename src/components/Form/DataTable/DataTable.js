...
export const DataTable = {
  ...
  watch: {
    isLoading() {
      if (this.isLoading && this.parentElement) {
        this.parentElement.removeEventListener('scroll', this.eventScrollPagination);
      } else {
        this.parentElement.addEventListener('scroll', this.eventScrollPagination);
        if (this.countDataTotal !== 0) setTimeout(() => this.eventScrollPagination(), 300);
      }
    },
    typeRow() {
      if (this.typeRow === 'dense') setTimeout(() => this.eventScrollPagination(), 300);
    },
    // propsTable() {
  ...
  methods: {
    ...
    async requestDataNext() {
      this.eventComponent('request:next');
    ...
    requestDataPrevious() {
      this.eventComponent('request:previous');
    },
    async addingNewElement() {
      ...
      //   guid: this.guid,
      // }).index;
      let index = this.isAddingMode.index;
      let sendOption = {
        tableName: this.tableName,
        guid: this.guid,
      };
      ...
      this.eventComponent('element:adding', sendOption);
    ...
    toggleGroup(options) {
      this.blurComponent();
      this.eventComponent('toggle:group', options);
