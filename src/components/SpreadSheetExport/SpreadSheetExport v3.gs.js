...
function exportAllSheetsJSON(type) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheets = ss.getSheets().slice(1, 2);
  ...
  delete exportDocument.sheetsList;
  ...
  for (var i = 0; i < countRow; i++) {
    for (let j = 0; j < countColumn; j++) {
      ...
      if (cellStyles) {
        ...
        exportDocument.styles[cellStyles.name] = cellStyles.list;
      }
...
function getCellStyle(sheet, borders, cellNameA1, sheetName) {
  ...
  if (borders && borders.userEnteredFormat) cellStyle.list = getCellBorders(borders);
