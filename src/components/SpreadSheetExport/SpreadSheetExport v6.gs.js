...
function onOpen() {
  ...
  const menuEntries = [
    ...
    { name: 'Export All lists', functionName: 'exportAllSheetsJSON' },
  ];
...
function exportAllSheetsJSON() {
  ...
  sheets.forEach((sheet) => {
    ...
    const countRow = values.length;
    const countColumn = values[0].length;

    const sheetId = sheet.getSheetId();
    const sheetName = `sheet${sheetId}`;
    const sheetNameView = sheet.getSheetName();
    exportDocument.sheets[sheetName] = {
      sheetId,
      sheetName,
      sheetNameView,
      rows: getRows(sheet, countRow),
      columns: getColumns(sheet, countColumn),
...
/**
 * @param {number} index sheet
 * @return {object} rows sheet
 */
function getRows(sheet, countRow) {
  const rows = {};
  // rowsFixed
  ...
  for (var i = sheet.getFrozenRows(); i < countRow; i++) {
    ...
    if (sheet.getRowGroupDepth(i + 1) > 0) {
      ...
      rows[i + 1].level = sheet.getRowGroupDepth(i + 1);
    }
  }
  console.log(rows);
  return rows;
...
function getColumns(sheet, countColumn) {
  const columns = {};
  ...
  for (var i = sheet.getFrozenColumns(); i < countColumn; i++) {
    ...
    if (sheet.getColumnGroupDepth(i + 1) > 0) {
      ...
      columns[columnParentName].level = sheet.getColumnGroupDepth(i + 1);
    }
  }
  console.log(columns);
  return columns;
...
function getTypeCell(cellNameA1) {
  ...
  console.log(cellName, formatCell);
